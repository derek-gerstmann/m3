...
/*#if defined(_DEBUG)
	...
	if(!node->IsLeaf()) {
		...
		// output the results
        std::cout << "Level:\tPages:\tRadius:\tArea:\n";
		for(i=maxn-1; i>=0; i--) {
			totn+=n[i];
			totarea+=areas[i];
            std::cout << i << ":\t" << n[i] << "\t" << radii[i]/n[i] << "\t" << areas[i]/n[i] << std::endl;
		}
        std::cout << "tot:\t" << totn << "\t" << totarea << std::endl;
		delete []areas;
		delete []radii;
		delete []n;
	}
...
MT::CheckNode(GiSTpath path, MTentry *e)
{
	...
	for(int i=0; (i<node->NumEntries())&&ret; i++) {
	    ...
	    if((e!=NULL)&&(((child->Key()->distance+child->maxradius())>e->maxradius())||(child->object().distance(e->object())!=child->Key()->distance))) {
            std::cout << "Error with entry " << child << "in " << node;
			ret=FALSE;
		}
...
MT::TopSearch(const TopQuery& query)
{
	...
	MTorderedlist<dst *> queue(comparedst);	// list for ordering the nodes
