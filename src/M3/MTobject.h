...
class MTobject : public GiSTobject	// the DB object class
{
public:
	double *x;	// the coords
//	long wasted_space[100];	// only for debug purposes
	MTobject() { x=new double[dimension]; };	// default constructor (needed)
	MTobject(const MTobject& obj) { x=new double[dimension]; for(int i=0; i<dimension; i++) x[i]=obj.x[i]; };	// copy constructor (needed)
	MTobject(char *key) {	x=new double[dimension]; memcpy(x, key, dimension*sizeof(double)); };	// member constructor (needed)
	MTobject(double *p_x) { x=new double[dimension]; for(int i=0; i<dimension; i++) x[i]=p_x[i]; };	// member constructor (needed)
	~MTobject() { delete []x; }	// destructor
	MTobject& operator=(const MTobject& obj) { for(int i=0; i<dimension; i++) x[i]=obj.x[i]; return *this; };	// assignment operator (needed)
	double area(double r) {	// only needed for statistic purposes (dependent on the metric, not applicable for non-vector metric spaces)
		double a=1;
	...
	int operator==(const MTobject& obj);	// equality operator (needed)
	int operator!=(const MTobject& obj) { return !(*this==obj); };	// inequality operator (needed)
	double distance(const MTobject& other) const;	// distance method (needed)
	int CompressedLength() const;	// return the compressed size of this object
...
double maxDist();	// return the maximum value for the distance between two objects
int sizeofObject();	// return the compressed size of each object (0 if objects have different sizes)
MTobject *Read();	// read an object from standard input
MTobject *Read(FILE *fp);	// read an object from a file
