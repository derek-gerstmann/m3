...
CommandCreate(const char *method, const char *table)
{
	...
	if(!strcmp(method, "mtree")) gist=new MT;
        ...
        std::cerr << "The only supported method is mtree.\n";
		return;
	}
	gist->Create(table);
	if(!gist->IsOpen()) {
		std::cout << "Error opening index.\n";
		delete gist;
		return;
	}
	std::cout << "Table " << table << " created as type " << method << ".\n";
}
...
{
	if (unlink(table)) std::cout << "There is no table by that name.\n";
	else std::cout << "Table " << table << " dropped.\n";
}
...
CommandOpen(const char *method, const char *table)
{
	...
	if(!gist->IsOpen()) {
		delete gist;
		std::cout << "Error opening table.\n";
		return;
	}
//	std::cout << "Table " << table << " opened.\n";
}
...
//	std::cout << key << "->" << ptr << " inserted into " << table << ".\n";
}
...
//	if(gist==NULL) printf("M3 NULL!!\n");
	printf("MinUtil=%f\n", MIN_UTIL);
	gist->BulkLoad(data, n, 1, (char*)("tmp"));
}
